...
> **Decision status note**: The ADR **decision status is Accepted** — the chosen option was implemented and verified in `ibn-core` PR #62 (`knowledge-graph-mcp-probe-patch.yaml`), which took the `knowledge-graph-mcp` pod from **151 restarts to 0** and is recorded as evidence in `docs/compliance/TMF921-CTK-v5-GateB-stubbed-2026-06-19.md` (PR #63). The **document-control Status is IN_REVIEW** because this ADR is a **retrospective ratification** of an already-merged operational fix, elevating it from a one-off PR to a programme-wide reliability standard — pending formal EARB sign-off. This is the "existing in-flight branches may be retro-fitted" path that `CLAUDE.md` (ArcKit-governs-all-new-work rule) explicitly permits.
...
**Approval Date**: 2026-09-20 (target ratification; decision already implemented per PR #62).
...
| Stamped at | 2026-06-20T20:55:28.867Z |
